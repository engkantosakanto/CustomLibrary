...
class AutomatonRobotLibrary(
    _ElementKeywords
    ):

    """AutomatonRobotLibrary is a ``gui recognition`` testing library for Robot Framework that leverages the ``SikuliX version 1.1.0`` methods 
    ...
    *Set the Environment Variables*
    ...
    = Installing the AutomatonRobotLibrary =
    | 1. Copy the ``AutomatonRobotLibrary`` to a local directory.                | 
    | 2. cd to the AutomatonRobotLibrary where ``setup.py`` is located           |
    | 3. Open a terminal or command prompt then install via ``jython setup.py install``. |

    = Verify Installation =
    Once you have installed AutomatonRobotLibrary it is a good idea to verify the installation. To verify installation start jython and then at the Jython prompt type:

    >>> import AutomatonRobotLibrary
    >>>
    ...
    All keywords in AutomatonRobotLibrary that need to find a ``pattern`` or any of the PSMRL on the screen take an argument that serves as the `locator` (for more information about ``PSMRL``, see http://sikulix-2014.readthedocs.org/en/latest/) . 
    ...
    *Using locators*
    ...
    def __init__(self
        ...
        """AutomatonRobotLibrary can be imported with optional arguments.
        ...
        libraries) to execute when a AutomatonRobotLibrary keyword fails. By default
        ...
        Examples:
        | Library `|` AutomatonRobotLibrary `|` 15                                                    | # Sets default sikulix_timeout to 15 seconds                                          |
        | Library `|` AutomatonRobotLibrary `|` 0 `|` 5                                               | # Sets default sikulix_timeout to 0 seconds and default sikulix_scanrate to 5 seconds |
        | Library `|` AutomatonRobotLibrary `|` 5 `|` run_on_failure=Capture Screenshot Of Active App | # Sets default timeout to 5 seconds and runs `Log Source` on failure                  |
        | Library `|` AutomatonRobotLibrary `|` sikulix_timeout=5       `|` run_on_failure=Log Source | # Sets default implicit_wait to 5 seconds and runs `Log Source` on failure            |
        | Library `|` AutomatonRobotLibrary `|` sikulix_scanrate=10     `|` run_on_failure=Nothing    | # Sets default sikulix_scanrate to 10 seconds and does nothing on failure             |
        """
        for base in AutomatonRobotLibrary.__bases__:
            base.__init__(self)